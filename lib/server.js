    ...
    self._getUtxosForCurrentWallet({}, function(err, utxos) {
      ...
      var info = {
       ...
       if(opts.isInstantSend===true){
        ...
        if(!validUTXO)
            return cb(new Error('InstantSend requires all Dinero you are sending (transaction inputs) to be held for about 15-20 minutes (minimum 6 network confirmations). Please wait before using InstantSend.'));
       }
 ...
 * @param {string} [opts.network = 'livenet'] - The Dinero network for this transaction.
